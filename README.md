# Your Name

*IT Support | Aspiring System Administrator | Future Cybersecurity Professional*

---

## Navigation

- 🏠 [Home](README.md)
- 📄 [Resume](resume.md)
- 💼 [Projects](projects.md)
- 👤 [About Me](about.md)
- ✉️ [Contact](contact.md)

---

# Welcome

Welcome to my professional IT portfolio.

This portfolio showcases my hands-on IT Support experience, home lab projects, technical documentation, and continuous learning as I work toward a career in IT Support and eventually Cybersecurity.

---

## Highlights

- CompTIA A+ Certified
- Active Directory Home Lab
- Windows Troubleshooting
- PowerShell Automation
- Technical Documentation
- Customer Service Experience

---

## Featured Projects

| Project | Description |
|---------|-------------|
| Active Directory Lab | Domain Controller, Users, Groups, GPO |
| Small Business Documentation | SOPs, KB Articles, Asset Inventory |
| PowerShell User Creation | Automated Active Directory user creation |

➡️ View all projects here:

[Projects](projects.md)
